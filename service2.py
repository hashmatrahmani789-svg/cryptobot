...
# ─────────────────────────────────────────
# SIGNAL 3 — Daily EMA 12/21 Cross
# ─────────────────────────────────────────
def check_daily_cross(symbol, coin, df):
    ...
    emoji = "🌙" if bullish else "🌑"
    txt   = "EMA12 crossed ABOVE EMA21" if bullish else "EMA12 crossed BELOW EMA21"
    bias  = "Daily bullish trend confirmed ✅" if bullish else "Daily bearish trend confirmed ⚠️"
...
def run():
    ...
    send_telegram(
        ...
        "💰 MC Filter   : > $1B\n"
        "🔄 Scan every  : 4H\n"
        "📊 No volume filter — pure price action"
    )
